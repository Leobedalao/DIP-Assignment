...
def create_mask_from_points(points, img_h, img_w):
    ...
    draw.polygon(clipped_points, outline=255, fill=255)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    lap_kernel = torch.tensor([[0.0, -1.0, 0.0],[-1.0, 4.0, -1.0],[0.0, -1.0, 0.0]],device=foreground_img.device,dtype=foreground_img.dtype).view(1, 1, 3, 3)
    lap_kernel = lap_kernel.repeat(3, 1, 1, 1)  # shape: [3, 1, 3, 3]
    ...
    mask = background_mask.expand(-1, 3, -1, -1)
    diff = (blended_lap - fg_lap) * mask
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    # Put foreground and foreground mask onto a background-sized canvas
    fg_on_bg = torch.zeros_like(bg_img_tensor)
    fg_mask_on_bg = torch.zeros_like(bg_mask_tensor)

    dx_i = int(dx)
    dy_i = int(dy)

    bg_h, bg_w = background_np.shape[0], background_np.shape[1]
    fg_h, fg_w = foreground_np.shape[0], foreground_np.shape[1]

    dst_x0 = max(0, dx_i)
    dst_y0 = max(0, dy_i)
    dst_x1 = min(bg_w, dx_i + fg_w)
    dst_y1 = min(bg_h, dy_i + fg_h)
    ...
    if dst_x1 > dst_x0 and dst_y1 > dst_y0:
        fg_on_bg[:, :, dst_y0:dst_y1, dst_x0:dst_x1] = fg_img_tensor[:, :, src_y0:src_y1, src_x0:src_x1]
        fg_mask_on_bg[:, :, dst_y0:dst_y1, dst_x0:dst_x1] = fg_mask_tensor[:, :, src_y0:src_y1, src_x0:src_x1]


    # Initialize blended image
    blended_img = bg_img_tensor.clone()
    mask_expanded = bg_mask_tensor.bool().expand(-1, 3, -1, -1)
    blended_img[mask_expanded] = blended_img[mask_expanded] * 0.9 + fg_on_bg[mask_expanded] * 0.1
    ...
    for step in range(iter_num):
        ...
        loss = cal_laplacian_loss(fg_on_bg, fg_mask_on_bg, blended_img_for_loss, bg_mask_tensor)
